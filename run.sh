...
echo "=> Backup started"
if mongodump ${HOST_STR}${PORT_STR}${USER_STR}${PASS_STR}${DB_STR} --archive=\${BACKUP_NAME} --gzip ${EXTRA_OPTS} && aws s3 cp \${BACKUP_NAME} \${S3BACKUP} ${REGION_STR} ${ENDPOINT_STR} && rm \${BACKUP_NAME} ;then
...
if aws s3 cp \${S3RESTORE} \${RESTORE_ME} ${REGION_STR} ${ENDPOINT_STR} && mongorestore ${HOST_STR}${PORT_STR}${USER_STR}${PASS_STR}${DB_STR} --drop --archive=\${RESTORE_ME} --gzip && rm \${RESTORE_ME}; then
